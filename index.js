...
bot.on('ready', () => {
  ...
  bot.user.setActivity('!!help for help | SAWO !', {
    type: "PLAYING"})
      .then(presence => console.log('your status has been set'))
      .catch(console.error);
...
bot.on("message", (message) => {
  if (message.content.startsWith("odading mang oleh")) {
    message.channel.send("RASANYA ANNNJIMM BANGETT !!!!");
  }
  if (message.content.startsWith("spontan")) {
    message.channel.send("UHUYYYY !!");
  }
  if (message.content.startsWith("ikan hiu")) {
    message.channel.send("ay lap yu tomat !!!!");
  }
  if (message.content.startsWith("ikan cupang makan tomat")) {
    message.channel.send("apasih bucin !!");
  }
});
...
bot.on("guildMemberAdd", (member) => {
  ...
  const newuserembed = new MessageEmbed()
            ...
            .setDescription('Welcome to **SAWO!** server !! Enjoy dude')
            .setThumbnail(member.user.displayAvatarURL())
            .setTimestamp()
            .setFooter("©️ 2020 SAWO ! Server");
  member.guild.channels.cache.find(c => c.name === "🚨welcome").send(newuserembed);
});
...
> \`nowplaying\`, \`queue\`, \`volume\``)
            .setFooter("©️ 2020 Satria Dewo");
        message.channel.send(helpembed);
    }
    if (message.content.startsWith(PREFIX + "ping")) {
      const helpembed = new MessageEmbed()
            .setColor("RED")
            .setDescription(":heartbeat: Your ping is `" + `${Date.now() - message.createdTimestamp}` + " ms`");
        message.channel.send(helpembed);
    }
...
async function handleVideo(video, message, voiceChannel, playlist = false) {
    ...
    if (!serverQueue) {
        ...
        if (playlist) return;
        else return message.channel.send(`☑️ **|**  **\`${song.title}\`** has been added to the queue`);
    }
    return;
}
